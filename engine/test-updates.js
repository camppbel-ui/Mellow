...
console.log('\nRefused');
{
  ...
  updates.install(makeZip(release('2026.09.21', { 'Mellow/Mellow.exe': 'MZ fake', 'Mellow/runtime/node.exe': 'MZ fake node',
    'Mellow/WebView2Loader.dll': 'MZ', 'Mellow/Microsoft.Web.WebView2.Core.dll': 'MZ', 'Mellow/.webview/EBWebView/x.js': 'x' })), opts);
  check('the app\'s launcher, Node.js and window are never written', !fs.existsSync(path.join(app, 'Mellow.exe')) && !fs.existsSync(path.join(app, 'runtime')) &&
    !fs.existsSync(path.join(app, 'WebView2Loader.dll')) && !fs.existsSync(path.join(app, 'Microsoft.Web.WebView2.Core.dll')) && !fs.existsSync(path.join(app, '.webview')));
  fs.rmSync(app, { recursive: true, force: true });
}
